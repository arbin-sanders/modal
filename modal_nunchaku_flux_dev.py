...
secrets = [modal.Secret.from_name("huggingface-app-secret", required_keys=["HF_TOKEN"])]
...
app = modal.App("nunchaku-flux-dev-fastapi")
...
class NunchakuFluxDevModel:
    ...
    def enter(self):
        ...
        # Apply cache optimizations for FLUX.1-dev
        apply_cache_on_pipe(
            self.pipe,
...
def fastapi_app():
    ...
    web_app = FastAPI(
        title="Nunchaku Flux Dev Generator",
        description="Generate images using Nunchaku-quantized Flux Dev model",
        version="1.0.0",
    )
    ...
    async def generate_image(
        ...
        """
        Generate an image using Nunchaku-quantized Flux Dev model.

        - **prompt**: Text description of the image you want to generate
        - **width**: Image width in pixels (default: 1024)
        - **height**: Image height in pixels (default: 1024)
        - **steps**: Number of denoising steps (default: 50, optimal for Dev)
        - **guidance_scale**: Guidance scale (default: 3.5, optimal for Dev)
        ...
        try:
            ...
            model = NunchakuFluxDevModel()
            result_bytes = model.inference.remote(
                prompt=request.prompt[:280],
                width=request.width,
            ...
            return Response(
                ...
                headers={
                    "Content-Disposition": f"inline; filename=nunchaku_flux_dev_{request.seed or 'random'}.png"
                },
            )
